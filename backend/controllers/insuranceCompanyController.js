...
exports.addInsuranceCompany = async (req, res) => {
  try {
    ...
    const existingCompany = await InsuranceCompany.findOne({ companyId });
    if (existingCompany) {
      return res.status(400).json({ message: "Company ID already exists" });
    }
    // Create new insurance company entry       
    const newCompany = new InsuranceCompany({ name, companyId, contactEmail, phoneNumber, address });
